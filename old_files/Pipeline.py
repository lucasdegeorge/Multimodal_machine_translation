...
def get_train_data_nouveau(batch_size, train_bool):
    ...
    train_data_fr = [
        ["DEBUT_DE_PHRASE"] + ligne.strip().split(" ") + ["FIN_DE_PHRASE"]
        for ligne in fichier_train_fr
    ]
    train_data_en = [
        ["DEBUT_DE_PHRASE"] + ligne.strip().split(" ") + ["FIN_DE_PHRASE"]
        for ligne in fichier_train_en
    ]
    ...
    train_data_fr = [
        [phrase[i] if i < len(phrase) else "TOKEN_VIDE" for i in range(longueur_max)]
        for phrase in train_data_fr
    ]
    train_data_en = [
        [phrase[i] if i < len(phrase) else "TOKEN_VIDE" for i in range(longueur_max)]
        for phrase in train_data_en
    ]
    ...
    tokenized_fr = torch.tensor(
        [[vocab_fr[mot] for mot in phrase] for phrase in train_data_fr]
    ).to(device=device, dtype=torch.long)
    tokenized_en = torch.tensor(
        [[vocab_en[mot] for mot in phrase] for phrase in train_data_en]
    ).to(device=device, dtype=torch.long)
...
def batchify(
    data: Tensor, bsz: int, image_bool, conservative=False, permute=True
) -> Tensor:
    ...
    if image_bool:
        ...
        if text_en.shape[0] % bsz != 0 and conservative:
            text_en = torch.cat(
                (
                    text_en,
                    torch.zeros(bsz - text_en.shape[0] % bsz, text_en.shape[1]).to(
            ...
            text_fr = torch.cat(
                (
                    text_fr,
                    torch.zeros(bsz - text_fr.shape[0] % bsz, text_fr.shape[1]).to(
                        device=device, dtype=torch.long
                    ),
                ),
                dim=0,
        ...
        # print(text.shape)
        return [
            text_en.view(text_en.shape[0] // bsz, bsz, text_en.shape[1]),
            features.view(
                features.shape[0] // bsz, bsz, features.shape[1], features.shape[2] ** 2
            ).transpose(-1, -2),
        ], [
            text_fr.view(text_fr.shape[0] // bsz, bsz, text_fr.shape[1]),
            features.view(
                features.shape[0] // bsz, bsz, features.shape[1], features.shape[2] ** 2
            ).transpose(-1, -2),
        ]
        ...
        return text_en.view(
            text_en.shape[0] // bsz, bsz, text_en.shape[1]
        ), text_fr.view(text_fr.shape[0] // bsz, bsz, text_fr.shape[1])
            ...
            if torch.equal(data[i][j:], padding_id * torch.ones_like(data[i][j:])):
                ...
                data[i] = torch.cat(
                    (
                        data[i][:j],
                        data[i][j + 1 :],
                        torch.Tensor([padding_id]).to(device=device, dtype=torch.int64),
                    )
                )
            else:
                j += 1
