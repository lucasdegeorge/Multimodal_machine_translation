...
def CCF_greedy(
    ...
    decoder_input = torch.cat(
        (
            torch.ones(batch_size, 1, dtype=torch.int).fill_(model_B.begin_id),
            torch.ones(batch_size, max_len - 1, dtype=torch.int).fill_(
                model_B.padding_id
            ),
        ),
        dim=1,
    ...
    for i in range(max_len - 1):
        tgt_mask = model_B.generate_square_subsequent_mask(
            model_B.n_head * decoder_input.shape[0], decoder_input.shape[1]
        )
        tgt_padding_mask = (decoder_input == model_B.padding_id).to(device=device)
        memory_mask = model_A.generate_square_subsequent_mask(
            decoder_input.shape[0], decoder_input.shape[1]
        )
        memory_key_padding_mask = (decoder_input == model_B.padding_id).to(
            device=device
        )
        ...
        if image_bool:
            mem_ei_mask = torch.zeros(
                [
                    ...
                    decoder_input.shape[1] + image_input.shape[1],
                ]
            ).to(device=device, dtype=bool)
            mem_ei_mask[
                :, 0 : decoder_input.shape[1], 0 : decoder_input.shape[1]
            ] = model_A.generate_square_subsequent_mask(
                decoder_input.shape[0], decoder_input.shape[1]
            ).to(
                device=device
            )
            mem_ei_key_padding_mask = (decoder_input == model_B.padding_id).to(
                device=device
            )
            mem_ei_key_padding_mask = torch.cat(
                (
                    mem_ei_key_padding_mask,
                    torch.full(
                        [decoder_input.shape[0], image_input.shape[1]], False
                    ).to(device=device),
                ),
                dim=1,
            )
        ...
        if image_bool:
            x = [
                model_B.positional_encoder(model_B.embedding(decoder_input.to(device))),
                image_encoded,
            ]
            if not get_attention:
                ...
                output, attention_e, attention_i = model_B.decoder(
                    x,
                    text_encoded,
                    tgt_mask,
                    mem_masks,
            ...
            if not get_attention:
                output = model_B.decoder(
                    model_B.positional_encoder(
                        model_B.embedding(decoder_input.to(device))
                    ),
                    x,
                    tgt_mask,
                    [memory_mask],
                ...
                output, attention_e = model_B.decoder(
                    x,
                    text_encoded,
                    tgt_mask,
                    mem_masks,
...
def CCF_beam_search(
    ...
    for i in range(max_len - 1):
        ...
        for seq, seq_score in beam:  # dim of seq : batch_size * i
            ...
            tgt_mask = model_B.generate_square_subsequent_mask(
                model_B.n_head * last_token.shape[0], last_token.shape[1]
            )
            tgt_padding_mask = (last_token == model_B.padding_id).to(device=device)
            memory_mask = model_A.generate_square_subsequent_mask(
                last_token.shape[0], last_token.shape[1]
            )
            memory_key_padding_mask = (last_token == model_B.padding_id).to(
                device=device
            )
            if image_bool:
                mem_ei_mask = torch.zeros(
                    [
                        last_token.shape[0],
                        last_token.shape[1],
                        last_token.shape[1] + image_input.shape[1],
                    ]
                ).to(device=device, dtype=bool)
                mem_ei_mask[
                    :, 0 : last_token.shape[1], 0 : last_token.shape[1]
                ] = model_A.generate_square_subsequent_mask(
                    last_token.shape[0], last_token.shape[1]
                ).to(
                    device=device
                )
                mem_ei_key_padding_mask = (last_token == model_B.padding_id).to(
                    device=device
                )
                mem_ei_key_padding_mask = torch.cat(
                    (
                        mem_ei_key_padding_mask,
                        torch.full(
                            [last_token.shape[0], image_input.shape[1]], False
                        ).to(device=device),
                    ),
                    dim=1,
                )
            ...
            if image_bool:
                x = [
                    model_B.positional_encoder(
                        model_B.embedding(last_token.to(device))
                    ),
                    image_encoded,
                ]
                if not get_attention:
            ...
            log_probs = torch.sum(
                torch.log_softmax(model_B.output_layer(output), dim=2)[:, : i + 1, :],
                dim=1,
            )
            top_k_scores, top_k_tokens = torch.topk(
                log_probs, k=beam_size, dim=-1, largest=True
            )  # Dim : [batch_size, beam_size]
            ...
            beam = [
                [
                    torch.ones(batch_size, max_len, dtype=torch.int).fill_(
                        model_B.padding_id
                    ),
                    torch.zeros(batch_size),
                ]
