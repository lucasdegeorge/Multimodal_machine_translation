    ...
    def forward(
        ...
        if image_bool:
            for i, mod in enumerate(self.layers):
                output, attention_weights_e, attention_weights_i = mod(
                    output,
                    memory,
                    tgt_mask=tgt_mask,
                    memory_mask=memory_mask,
                ...
                if i == 0:
                    ...
                    attention_weights_i_sum += attention_weights_i.sum(dim=1)
            output = output[0]
            attention_weights_e_sum = attention_weights_e_sum / (
                self.num_layers * n_heads
            )
            attention_weights_i_sum = attention_weights_i_sum / (
                self.num_layers * n_heads
            )
            if self.norm is not None:
                output = self.norm(output)
            ...
            for i, mod in enumerate(self.layers):
                output, attention_weights_e = mod(
                    output,
                    memory,
                    tgt_mask=tgt_mask,
                    memory_mask=memory_mask,
