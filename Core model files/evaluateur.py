...
def give_tokens(
    output, padding_id, end_id
):  # takes output of greedy search tensor of size [bsz,seqlen,n_token]. returns the tokens, with no padding before end of sentence token
...
def traduit(mode, model_A, model_B, src, inv_map_src, image_bool, tgt, inv_map_tgt, j):
    ...
    with open(model_A.prefix + "logs.txt", "a") as logs:
        if image_bool:
            if mode == "greedy":
                output = give_tokens(
                    CCF_greedy(model_A, model_B, data, features, True),
                    model_B.padding_id,
                    model_B.end_id,
                )[j]
            else:
            ...
            if mode == "greedy":
                output = give_tokens(
                    CCF_greedy(model_A, model_B, data, None, False),
                    model_B.padding_id,
                    model_B.end_id,
                )[j]
            else:
        ...
        if image_bool:
            logs.write(
                str(
                    tensor_to_sentence(
                        torch.argmax(model_A(data, True, features), dim=2)[j].view(-1),
                        inv_map_src,
                    )
                )
            )
            ...
            logs.write(
                str(
                    tensor_to_sentence(
                        torch.argmax(model_A(data, False), dim=2)[j].view(-1),
                        inv_map_src,
                    )
                )
            )
...
def evalue_bleu(output, inv_dic_tgt, tgt):
    ...
    result = [
        inv_dic_tgt[int(x)]
        for x in output
        if inv_dic_tgt[int(x)] not in ["TOKEN_VIDE", "DEBUT_DE_PHRASE", "FIN_DE_PHRASE"]
    ]
    target = [
        inv_dic_tgt[int(x)]
        for x in tgt
...
def donne_random(i, j, val_data_en, val_data_fr, batch_size, image_bool):
    batched_data_en, batched_data_fr = batchify(
        [val_data_en, val_data_fr], batch_size, image_bool
    )
    if image_bool:
...
def evaluation(
    ...
    if image_bool:
        src, features, tgt = donne_random(
            i, j, val_data_en, val_data_fr, batch_size, image_bool
        )
    ...
    trad, bleu, meteor = traduit(
        mode, model_en, model_fr, data, inv_map_en, image_bool, tgt[j], inv_map_fr, j
    )
...
def dataframe_eval(
    ...
    for batch in range(len(src)):
        print(batch)
        if image_bool:
            src[batch], features[batch], tgt[batch] = (
                src[batch].to(device),
                features[batch].to(device),
                tgt[batch].to(device),
            )
            if mode == "greedy":
                traduction = torch.argmax(
                    CCF_greedy(
                        model_en, model_fr, src[batch], features[batch], image_bool
                    ),
                    dim=2,
                )
            else:
                traduction = CCF_beam_search(
                    model_en,
                    model_fr,
                    src[batch],
                    beam_size=3,
                    image_input=features[batch],
                    image_bool=True,
                    get_attention=False,
                )
            for i in range(traduction.shape[0]):
                temp = cut_list_at_value(
                    [
                        inv_map_fr[traduction[i][j].item()]
                        for j in range(traduction.shape[1])
                    ],
                    "FIN_DE_PHRASE",
                )
                traductions_en_fr.append(
                    [
                        x
                        for x in temp
                        if x not in ["TOKEN_VIDE", "DEBUT_DE_PHRASE", "FIN_DE_PHRASE"]
                    ]
                )
                references_fr.append(
                    [
                        inv_map_fr[tgt[batch][i][j].item()]
                        for j in range(tgt[batch].shape[1])
                        if inv_map_fr[tgt[batch][i][j].item()]
                        not in ["TOKEN_VIDE", "DEBUT_DE_PHRASE", "FIN_DE_PHRASE"]
                    ]
                )
            if mode == "greedy":
                traduction = torch.argmax(
                    CCF_greedy(
                        model_fr, model_en, tgt[batch], features[batch], image_bool
                    ),
                    dim=2,
                )
            else:
            ...
            for i in range(traduction.shape[0]):
                temp = cut_list_at_value(
                    [
                        inv_map_en[traduction[i][j].item()]
                        for j in range(traduction.shape[1])
                    ],
                    "FIN_DE_PHRASE",
                )
                traductions_fr_en.append(
                    [
                        x
                        for x in temp
                        if x not in ["TOKEN_VIDE", "DEBUT_DE_PHRASE", "FIN_DE_PHRASE"]
                ...
                references_en.append(
                    [
                        inv_map_en[src[batch][i][j].item()]
                        for j in range(src[batch].shape[1])
                        if inv_map_en[src[batch][i][j].item()]
                        not in ["TOKEN_VIDE", "DEBUT_DE_PHRASE", "FIN_DE_PHRASE"]
                    ]
                )
            if mode == "greedy":
                traduction = torch.argmax(
                    CCF_greedy(model_en, model_fr, src[batch], None, False), dim=2
                )
            else:
                traduction = CCF_beam_search(
                    model_en,
            ...
            for i in range(traduction.shape[0]):
                ...
                traductions_en_fr_txt_only.append(
                    [
                        x
                        for x in temp
                        if x not in ["TOKEN_VIDE", "DEBUT_DE_PHRASE", "FIN_DE_PHRASE"]
            ...
            if mode == "greedy":
                traduction = torch.argmax(
                    CCF_greedy(model_fr, model_en, tgt[batch], None, False), dim=2
                )
            else:
                traduction = CCF_beam_search(
                    model_fr,
            ...
            for i in range(traduction.shape[0]):
                ...
                traductions_fr_en_txt_only.append(
                    [
                        x
                        for x in temp
                        if x not in ["TOKEN_VIDE", "DEBUT_DE_PHRASE", "FIN_DE_PHRASE"]
            ...
            if mode == "greedy":
                traduction = torch.argmax(
                    CCF_greedy(model_en, model_fr, src[batch], None, image_bool), dim=2
                )
            else:
                traduction = CCF_beam_search(
                    model_en,
            ...
            if mode == "greedy":
                traduction = torch.argmax(
                    CCF_greedy(model_fr, model_en, tgt[batch], None, image_bool), dim=2
                )
            else:
                traduction = CCF_beam_search(
                    model_fr,
    ...
    if image_bool:
        data = {
            "traductions_en_fr": traductions_en_fr,
            "references_fr": references_fr,
            "traductions_fr_en": traductions_fr_en,
            "references_en": references_en,
            "traductions_en_fr_txt_only": traductions_en_fr_txt_only,
            "traductions_fr_en_txt_only": traductions_fr_en_txt_only,
        }
    else:
        data = {
            "traductions_en_fr": traductions_en_fr,
...
def save_dataframe_eval(
    ...
    if image_bool:
        df["bleu_en_fr_txt_only"] = df.apply(
            lambda row: bleu_txt_only(row, "en"), axis=1
        )
        df["bleu_fr_en_txt_only"] = df.apply(
            lambda row: bleu_txt_only(row, "fr"), axis=1
        )
...
def graphiques_attention(
    ...
    for batch in range(len(src)):
        ...
        traduction, attention_e, attention_i = CCF_greedy(
            model_en, model_fr, src[batch], features[batch], True
        )
        traduction = torch.argmax(traduction, dim=2)
        for i in range(traduction.shape[0]):
            # Attention text
            phrase = cut_list_at_value(
                [
                    inv_map_en[src[batch][i][j].item()]
                    for j in range(src[batch].shape[1])
                ],
